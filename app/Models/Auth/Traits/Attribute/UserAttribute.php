...
trait UserAttribute
{
    ...
    public function getLoginAsButtonAttribute()
    {
        ...
        if (! session()->has('admin_user_id') || ! session()->has('temp_user_id')) {
            ...
            if ($this->id != auth()->id()) {
                ...
                return '<a href="'.route('admin.auth.admin.login-as',
                        $this).'" class="dropdown-item">'.__('buttons.backend.access.admins.login_as', ['user' => $this->full_name]).'</a> ';
                  else
    ...
    public function getClearSessionButtonAttribute()
    {
        if ($this->id != auth()->id() && config('session.driver') == 'database') {
              if($this->sponsor_id == null)
            return '<a href="'.route('admin.auth.admin.clear-session', $this).'"
			 	 data-trans-button-cancel="'.__('buttons.general.cancel').'"
                 data-trans-button-confirm="'.__('buttons.general.continue').'"
                 data-trans-title="'.__('strings.backend.general.are_you_sure').'"
                 class="dropdown-item" name="confirm_item">'.__('buttons.backend.access.admins.clear_session').'</a> ';
              else
    ...
    {
        if($this->sponsor_id == null)
        return '<a href="'.route('admin.auth.admin.show', $this).'" class="btn btn-info"><i class="fa fa-search" data-toggle="tooltip" data-placement="top" title="'.__('buttons.general.crud.view').'"></i></a>';
        else
    ...
    {
        if($this->sponsor_id == null)
            return '<a href="'.route('admin.auth.admin.edit', $this).'" class="btn btn-primary"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="'.__('buttons.general.crud.edit').'"></i></a>';
        else
    ...
    {
        if($this->sponsor_id == null)
        return '<a href="'.route('admin.auth.admin.change-password', $this).'" class="dropdown-item">'.__('buttons.backend.access.admins.change_password').'</a> ';
        else
    ...
    public function getConfirmedButtonAttribute()
    {
        if (! $this->isConfirmed() && ! config('access.users.requires_approval')) {
             if($this->sponsor_id == null)
            return '<a href="'.route('admin.auth.admin.account.confirm.resend', $this).'" class="dropdown-item">'.__('buttons.backend.access.admins.resend_email').'</a> ';
            else
    ...
    public function getDeleteButtonAttribute()
    {
        if ($this->id != auth()->id() && $this->id != 1) {
             if($this->sponsor_id == null)
            return '<a href="'.route('admin.auth.admin.destroy', $this).'"
                 data-method="delete"
                 data-trans-button-cancel="'.__('buttons.general.cancel').'"
    ...
    {
          if($this->sponsor_id == null)
        return '<a href="'.route('admin.auth.admin.delete-permanently', $this).'" name="confirm_item" class="btn btn-danger"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="'.__('buttons.backend.access.admins.delete_permanently').'"></i></a> ';
        else
    ...
    {
          if($this->sponsor_id == null)
        return '<a href="'.route('admin.auth.admin.restore', $this).'" name="confirm_item" class="btn btn-info"><i class="fa fa-refresh" data-toggle="tooltip" data-placement="top" title="'.__('buttons.backend.access.admins.restore_user').'"></i></a> ';
        else
