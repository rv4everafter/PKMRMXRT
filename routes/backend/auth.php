<?php

/**
 * All route names are prefixed with 'admin.auth'.
 */
Route::group([
    'prefix'     => 'auth',
    'as'         => 'auth.',
    'namespace'  => 'Auth',
], function () {
    Route::group([
        'middleware' => 'role:superadmin',
    ], function () {
         /*
         * Admin Management
         */
        Route::group(['namespace' => 'Admin'], function () {

            /*
             * Admin Status'
             */
            Route::get('admin/deactivated', 'AdminStatusController@getDeactivated')->name('admin.deactivated');
            Route::get('admin/deleted', 'AdminStatusController@getDeleted')->name('admin.deleted');

            /*
             * Admin CRUD
             */
            Route::resource('admin', 'AdminController');

            /*
             * Specific Admin
             */
            Route::group(['prefix' => 'admin/{admin}'], function () {
                // Account
                Route::get('account/confirm/resend', 'AdminConfirmationController@sendConfirmationEmail')->name('admin.account.confirm.resend');

                // Status
                Route::get('mark/{status}', 'AdminStatusController@mark')->name('admin.mark')->where(['status' => '[0,1]']);

                // Social
                Route::delete('social/{social}/unlink', 'AdminSocialController@unlink')->name('admin.social.unlink');

                // Confirmation
                Route::get('confirm', 'AdminConfirmationController@confirm')->name('admin.confirm');
                Route::get('unconfirm', 'AdminConfirmationController@unconfirm')->name('admin.unconfirm');

                // Password
                Route::get('password/change', 'AdminPasswordController@edit')->name('admin.change-password');
                Route::patch('password/change', 'AdminPasswordController@update')->name('admin.change-password.post');

                // Access
                Route::get('login-as', 'AdminAccessController@loginAs')->name('admin.login-as');

                // Session
                Route::get('clear-session', 'AdminSessionController@clearSession')->name('admin.clear-session');
            });

            /*
             * Deleted Admin
             */
            Route::group(['prefix' => 'admin/{deletedUser}'], function () {
                Route::get('delete', 'AdminStatusController@delete')->name('admin.delete-permanently');
                Route::get('restore', 'AdminStatusController@restore')->name('admin.restore');
            });
        });
        /*
         * User Management
         */
        Route::group(['namespace' => 'User'], function () {

            /*
             * User Status'
             */
            Route::get('user/deactivated', 'UserStatusController@getDeactivated')->name('user.deactivated');
            Route::get('user/deleted', 'UserStatusController@getDeleted')->name('user.deleted');

            /*
             * User CRUD
             */
            Route::resource('user', 'UserController');

            /*
             * Specific User
             */
            Route::group(['prefix' => 'user/{user}'], function () {
                // Account
                Route::get('account/confirm/resend', 'UserConfirmationController@sendConfirmationEmail')->name('user.account.confirm.resend');

                // Status
                Route::get('mark/{status}', 'UserStatusController@mark')->name('user.mark')->where(['status' => '[0,1]']);

                // Social
                Route::delete('social/{social}/unlink', 'UserSocialController@unlink')->name('user.social.unlink');

                // Confirmation
                Route::get('confirm', 'UserConfirmationController@confirm')->name('user.confirm');
                Route::get('unconfirm', 'UserConfirmationController@unconfirm')->name('user.unconfirm');

                // Password
                Route::get('password/change', 'UserPasswordController@edit')->name('user.change-password');
                Route::patch('password/change', 'UserPasswordController@update')->name('user.change-password.post');

                // Access
                Route::get('login-as', 'UserAccessController@loginAs')->name('user.login-as');

                // Session
                Route::get('clear-session', 'UserSessionController@clearSession')->name('user.clear-session');
            });

            /*
             * Deleted User
             */
            Route::group(['prefix' => 'user/{deletedUser}'], function () {
                Route::get('delete', 'UserStatusController@delete')->name('user.delete-permanently');
                Route::get('restore', 'UserStatusController@restore')->name('user.restore');
            });
        });

        /*
         * Role Management
         */
        Route::group(['namespace' => 'Role'], function () {
            Route::resource('role', 'RoleController', ['except' => ['show']]);
        });
    });
});
